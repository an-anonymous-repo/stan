...
from src.nn_classifier import task1_classifier
import os
...
exp_setting_list = [[True, ''], [False, 'bsl1'], [False, 'bsl2'], [False, 'wpgan'], [False, 'ctgan']]#, [False, 'arcnn']]#, [False, 'trainreal']]
exp_setting_list = [[False, 'bsl1'], [False, 'bsl2']]
#exp_setting_list = [[False, 'arcnn']]
#exp_setting_list = [[False, 'ctgan']]
result_record = 'results/tasks_record_task1_disturb_bsl.txt' 
...
def fold_i_of_k(df_real,df_synth, real_percent, no_synth, i, k):
    ...
    if real_percent <= 1:
        print('frac mode reduction,', real_percent)
        if no_synth:
            train_ = train_.sample(frac=real_percent)
            print('train_ amount', len(train_.index))
        else:
            synth_amount = int(len(train_.index) * (1-real_percent))
            p_train = train_.sample(frac=real_percent)
            p_synth = df_synth.sample(n=synth_amount)
            p_synth['pr'] = np.random.permutation(p_synth['pr'].values) 
...
def mixed_cross_validation(df_real, df_synth, synth_percent,no_synth=False):
    ...
    for i_fold in range(k_fold):
        ...
        train_[train_.columns] = train_[train_.columns].applymap(np.int64).dropna()
        test_[test_.columns] = test_[test_.columns].applymap(np.int64).dropna()
        def onehot_pr(oh_df):
            mycol = 'pr-1'
            df_prime = pd.concat([oh_df,pd.get_dummies(oh_df[mycol], prefix=mycol,dummy_na=True)],axis=1).drop([mycol],axis=1)
            df_prime = df_prime.drop(['pr-1_nan'],axis=1)
        #    df_prime = df_prime.drop(['byt-1'],axis=1)
        #    df_prime = df_prime.drop(['pkt-1'],axis=1)
            return df_prime

        train_ = onehot_pr(train_)
        test_ = onehot_pr(test_)
        ...
        if the_task == 'task1':
            ...
            #clf = task1_classifier().scale(train_X, test_X).fit(train_X, train_y)
            #clf = task1_classifier().fit(train_X, train_y)
...
def run(exp_setting, piece_i):
    ...
    #print(df_synth[:10])
    #print('for synth pearson coef\n', df_synth.corr(method ='pearson'))
    #input()
    for i in iter_list:
        frac = i
